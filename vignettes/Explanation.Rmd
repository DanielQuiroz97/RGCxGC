...
csl: analitica-chimica-acta.csl
output:
...
# Introduction
This is the vignette to explain the implementation of RGCxGC package. This text presents an end to end pipeline for the analysis for comprehensive two
...
Figure 1. Raw chromatographic signals ussually contains undesirable artifacts,
...
herein smoothing, baseline correction, and peak alignment. Then, in order to differentiate between groups/treatments, multivariate analysis can be performed.

```{r Fig 1, echo=FALSE, fig.cap= "Figure 1. Overview of general data processing in chromatography that is presented in the RGCxGC package.", out.width= "80%",message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center'}
...
The raw NetCDF file is imported with **read_chrom** function, by providing
the modulation time that the GCxGC data was acquiered.
...
respectively. Then, peak alignment one sample against a reference chromatogram
can be performed based on the two-dimensional
correlation optimized warping (2DCOW) algorithm. Alternatively, mutiple sample
alignment can be performed with **batch_2DCOW** function. This function takes
the first chromatogram as the reference.
After signal preprocessing, MPCA can be performed over the dataset by using
the **m_prcomp** functions. As result of the MPCA, you can acces to the score
...
the **print** function. On the other hand, the RGCxGC package can export the
...
[mixOmics](http://mixomics.org/s) functions.
...
Another way is to install it through the developer version in github.
...
the modulation time. This is an adaptation of @Skov2008 routine.
...
slotNames(MTBLS08)
```
As one can see, the MTBLS08 object has four slots. The first one correspond to the
location of the imported two-dimensional chromatogram. The second slot refers to
...
It is built from **filled.contour** r base function. Since countour plots
...
For visualizing a familiar chromatogram, you have to provide the color palette and
...
disturbs the chemical interpretation of the resutls. In order to remove this irrelevant information, some
...
the penalty order (1 or 2) and the $\lambda$ arguments has to be provided.
...
Then, greater parameter values will hava a stronger influence in the smmothing
process.
...
using the two-dimensional correlation optimized warping (2D-COW) algorithm
...
warping (COW) [@Tomasi2004].

The **TwoDCOW** function need four arguments: sample the reference chromatogram, the number of segments to split the chromatogram and the maximum
...
The batch funcion needs a **named list** of sample chromatograms that will be
aligned. Just for demostration GB08 chromatogram will be triplicated to simulate
multiple samples.
...
As you can see, the names of the sample chromatograms are the names of the list.
...
By defautl, the MPCA is carried out with mean centered and unit scaled data.
...
Then, the user should be proceed as follow:
...
(chromatographic) differences among the samples. To plot access the scores
