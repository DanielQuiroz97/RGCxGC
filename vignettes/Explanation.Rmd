...
```{r Chrom plot, message=FALSE, warning=FALSE, paged.print=FALSE, out.width= "80%"}
...
```{r baseline correction, out.width= "80%"}
...
```{r batch alignment}
St_carriege <- batch_2DCOW(chrom_fl, mod_time = 5L, segments = c(10, 40),
                            max_warp = c(1, 8))
```
...
(type = "n") loading values. The default option of this fucntion is to plot the
firts principal component, even though, you can choose setting the *pc* argument.

```{r positive loadings, out.width= "80%"}
# Positive loadings
plot_loading(exp_MPCA, type = "p", nlevels = 100, main = "Positive loadings",
             color.palette = colorRamps::matlab.like)
```

```{r negative loadings, out.width= "80%"}
# Negative loadings
plot_loading(exp_MPCA, type = "n", nlevels = 100, main = "Negative loadings",
             color.palette = colorRamps::matlab.like)
```
