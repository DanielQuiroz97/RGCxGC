...
of points that the detector is capable to meassure per second. Each discrete
point contributes to create a continous time signal. A single run can be
discribed as $A_{i}$ where $i$ is the number of scan points in the
chromatographic run.

This is not the case of comprehensive chromatography (GCxGC). In the case of
GCxGC, there is a second column conected with the first column by a modulator.
The chemistry of the second column is usually different than the first column,
some authors claims this difference as ortogonal separation.
Altough, it can not alway be achieved since the chemistry of both columns
is not opposite 100%. Therefore, the second column is considered as a
second chromatographic dimension. Thus, a single two-dimensional chromatogram
can be discribed as a set of modulations $k$, where each modulation has a
...
The color palett of this chromatogram might be suitable, or the common
color palette employed in some softwares like MATLAB or GCImage. One of 
...
Befote to change the chromatogram color palette, we would like to make emphasis
that the back-end function employed in this plot, is the `filled.contour`
function. To change the palete, we have to provide the function that creates
the palette.
...
We spot some similarities between this two palettes. Firt of all, the
palette button and apper limits are very similar. Even though, the
...
palette the variation lower signals intensites is obscured by a blue
...
# TOF vs. Quadrupole
The MS detector comes in some flavors, which two of the most common are the
Time of flight and Quadrupole mass analyzers. They are very differents about
how they work and the data quality. For example, TOF mas analyzer can acquiere
...
quadrupole. Therefore, chromatograms acquiered with TOF mass analyzer are
usually dimensionally higher ($> i$). 

The previous chromatogram was acquiered with TOF mass analyzer, thus
...
We are going to addres this issues in four steps:
...
peak volumen in the modulation $i$, and reinjects the remaining peak in the
next modulation $i+1$. The first chromatogram that we plot in the section 4
presents this challange. Therefore, in order to improve the chromatogram
...
As aforementioned, the defalt plot type is the filled contour. Even though,
compound concentration is usually uneven in untargeted analysis. Therefore
...
were obsucred by the backgroud signal.
...
colors will be used as changing breaks.
...
set this argumnent to a greater value, the total TIC signal will be splitted
in more braks, which enhance the feature visualization.
